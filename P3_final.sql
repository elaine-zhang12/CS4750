...
-- Top 3 highest rated books (JOIN) with ties
SELECT TOP 3 WITH TIES Title, AVG(Rating) AS AvgRating
FROM Books
JOIN Reviews ON Books.ItemID = Reviews.ItemID
GROUP BY Title
ORDER BY AvgRating DESC;

-- Total reservations for top 5 most reserved books (Subquery) with ties
SELECT Title, ReservationCount
FROM Books
JOIN LibraryItemState ON Books.ItemID = LibraryItemState.ItemID
WHERE Books.ItemID IN (
    SELECT TOP 5 WITH TIES ItemID
...
SELECT TOP 5 WITH TIES Name, NumReserved
FROM LibraryAccount
